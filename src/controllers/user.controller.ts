...
import { sendOtpEmail } from "../utils/otp-email.util";
...
export const sendOtp = asyncHandler(async (req: Request, res: Response) => {
  ...
  // Gửi email với mã OTP sử dụng template
  ...
  if (!emailSent) {
    return sendErrorResponse(res, "Không thể gửi email OTP", 500);
  }
...
/**
 * @desc    Reset mật khẩu với email và mật khẩu cũ
...
export const resetPassword = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    try {
      // Tìm người dùng theo email và bao gồm trường password
      const user = await User.findOne({ email }).select("+password");
      ...
      await sendEmail({
        ...
        text: `Mật khẩu của bạn đã được thay đổi thành công vào lúc ${new Date().toLocaleString(
          "vi-VN"
        )}. Nếu bạn không thực hiện thay đổi này, vui lòng liên hệ với chúng tôi ngay lập tức.`,
      });

      return sendSuccessResponse(res, "Reset mật khẩu thành công");
    } catch (error :any) {
